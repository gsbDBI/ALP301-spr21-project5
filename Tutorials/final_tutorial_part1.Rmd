...
logged_df <- logged_df %>% 
  dplyr::rename('child_id_code' = child_id, 'child_id' = child_id_code)
...
first_six_weeks = stories_per_week %>%
  dplyr::filter(weeks_retained < 7) #edit here 
...
```{r}
# Get only the first ten weeks of data
first_ten_weeks = stories_per_week %>%
  dplyr::filter(weeks_retained < 11) 

# Calculate proportion of users retain by weeks since first signup
# by sign-up date
retention_df_frst_ten_weeks_by_signup_date = first_ten_weeks %>%
  group_by(weeks_retained, created_at_year_month) %>%
  summarize(num_users = mean(retained_user))

# Plot retention curves
ggplot(retention_df_frst_ten_weeks_by_signup_date %>%
         dplyr::filter(created_at_year_month < "20-12", weeks_retained > 0)) +
  geom_line(aes(x = weeks_retained, y = num_users, color = created_at_year_month)) +
...
```{r}
# Get the max weeks of data
first_max_weeks = stories_per_week %>%
  dplyr::filter(weeks_retained < max(weeks_retained))  

# Calculate proportion of users retain by weeks since first signup
# by sign-up date
retention_df_frst_max_weeks_by_signup_date = first_max_weeks %>%
  group_by(weeks_retained, created_at_year_month) %>%
  summarize(num_users = mean(retained_user))

# Plot retention curves
ggplot(retention_df_frst_max_weeks_by_signup_date %>%
         dplyr::filter(created_at_year_month < "20-12", weeks_retained > 0)) +
  geom_line(aes(x = weeks_retained, y = num_users, color = created_at_year_month)) +
  scale_x_continuous(breaks=seq(1, as.integer(max(stories_per_week$weeks_retained)),by=3)) +
  xlab("Weeks since first activity") + 
  ylab("Proportion of Retained Users")  +
  labs(color = "Cohort: sign-up month") + 
  theme_bw() +
...
# Getting grade and acquisition channel information 
b2_grade<-filtered_child_df %>% 
  select(child_id, user, grade)

first_six_weeks_b2_grade<-first_six_weeks %>% 
  left_join(b2_grade, by="child_id")

# Calculate proportion of users retain by weeks since first signup
# by user-acquisition method
retention_df_frst_six_weeks_by_b2 = first_six_weeks_b2_grade %>%
  group_by(weeks_retained, user) %>%
  summarize(num_users = mean(retained_user))

# Plot retention curves
ggplot(retention_df_frst_six_weeks_by_b2 %>%
         dplyr::filter(weeks_retained > 0)) +
  geom_line(aes(x = weeks_retained, y = num_users, color = user)) +
  scale_x_continuous(breaks=seq(1, 6)) +
  xlab("Weeks since first activity") + 
  ylab("Proportion of Retained Users")  +
  labs(color = "Cohort: user acquisition method") + 
  theme_bw() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank())
...
# Calculate proportion of users retain by weeks since first signup
# by sign-up date
retention_df_frst_six_weeks_by_grade = first_six_weeks_b2_grade %>%
  group_by(weeks_retained, grade) %>%
  summarize(num_users = mean(retained_user))

# Plot retention curves
ggplot(retention_df_frst_six_weeks_by_grade %>%
         dplyr::filter(weeks_retained > 0)) +
  geom_line(aes(x = weeks_retained, y = num_users, color = grade)) +
  scale_x_continuous(breaks=seq(1, 6)) +
  xlab("Weeks since first activity") + 
  ylab("Proportion of Retained Users")  +
  labs(color = "Cohort: grade") + 
  theme_bw() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank())
...
```{r prop_user_retention_by_quartiles_maxwks}
# Get the number of stories read in the first week of being active on app
num_stories_first_week_max = first_max_weeks %>%
  dplyr::filter(weeks_retained == 0)

# Split the users into 4 quartiles based on the number of stories during their first week of app use
num_stories_first_week_max$n_stories_quartile_cohort = factor(ntile(num_stories_first_week_max$num_stories, n=4)) 

# Add the quartile label to the first_six_weeks dataframe
first_max_weeks = left_join(first_max_weeks, num_stories_first_week_max %>% select(child_id, n_stories_quartile_cohort),
          by = "child_id")

# Calculate proportion of users retain by weeks since first signup
# by quartile
retention_df_frst_max_weeks_by_num_stories_frst_week_max = first_max_weeks %>%
  group_by(weeks_retained, n_stories_quartile_cohort) %>%
  summarize(num_users = mean(retained_user))

# Plot retention curves
ggplot(retention_df_frst_max_weeks_by_num_stories_frst_week_max %>%
         dplyr::filter(weeks_retained > 0)) +
  geom_line(aes(x = weeks_retained, y = num_users, color = n_stories_quartile_cohort)) +
  scale_x_continuous(breaks=seq(1, as.integer(max(stories_per_week$weeks_retained)),3)) + 
  xlab("Weeks since first activity") + 
  ylab("Proportion of Retained Users")  +
  labs(color = "Cohort: number of stories \n in first week quartile") + 
  theme_bw() +
...
```{r prop_user_retention_by_quartiles_six6ks2}
# Get the number of stories read in the first week of being active on app
num_stories_first_week = first_six_weeks %>%
...
# by quartile
retention_df_frst_six_weeks_by_num_stories_frst_week = first_six_weeks %>%
  group_by(weeks_retained, n_stories_quartile_cohort.y) %>%
  summarize(num_users = mean(retained_user))

# Plot retention curves
ggplot(retention_df_frst_six_weeks_by_num_stories_frst_week %>%
         dplyr::filter(weeks_retained > 0)) +
  geom_line(aes(x = weeks_retained, y = num_users, color = n_stories_quartile_cohort.y)) +
  scale_x_continuous(breaks=seq(1, 6)) + 
...
```{r avg_active_days_weekly_6wks2}
# Average number of active days week over week in first 6 weeks since signup
users_compo_frst_six_week_active_days <- filtered_logged_df_may_nov %>%
  dplyr::filter(sessions_since_signup <= 5) %>% # note: first week of signup is 0 
